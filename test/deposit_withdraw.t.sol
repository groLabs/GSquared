...
contract TrancheTest is Test, BaseSetup {
    ...
    function testSeniorTrancheLosses(uint256 _deposit, uint16 _change, uint256 _i, bool _loss) public {
        ...
        console2.log('vault 1 assets %s strategy assets %s', gVault.totalAssets(), convexStrategy.estimatedTotalAssets());
        ...
        console2.log('vault 2 assets %s strategy assets %s', gVault.totalAssets(), convexStrategy.estimatedTotalAssets());
        console2.log('crv assets vault %s strategy assets %s', THREE_POOL_TOKEN.balanceOf(address(gVault)), ERC20(fraxConvexRewards).balanceOf(address(convexStrategy)));
        vm.startPrank(alice);
        ...
        initialSeniorTrancheAssets = initialSeniorTrancheAssets - aliceSeniorAssets/10;
        vm.stopPrank();
    ...
    ///  for all gain/losses in senior and junior tranche in the set of tranche asset changes,
    ///  the senior tranche can only be be reduced if and only if the junior tranche is 0
    function testJuniorGetsAllYield(uint256 deposit, uint16 _change, uint256 i) public {
        ...
        setStorage(address(convexStrategy), THREE_POOL_TOKEN.balanceOf.selector, address(THREE_POOL_TOKEN), gain);
        ...
        gRouter.withdraw(aliceSeniorAssets / 10, 0, true, 0);
        initialSeniorTrancheAssets = initialSeniorTrancheAssets - aliceSeniorAssets/10;
        initialJuniorTrancheAssets = initialJuniorTrancheAssets - aliceJuniorAssets/10;
        vm.stopPrank();
    ...
    function testwithdrawNoImpact(uint256 deposit, uint8 _i, uint8 _k) public {
        ...
        for(uint256 j; j < i; j++) {
            (uint256 withdrawnSenior, uint256 withdrawnJunior) = runWithdrawal(users[j], k);
            totalWithdrawnSenior += withdrawnSenior;
            totalWithdrawnJunior += withdrawnJunior;
        }

        assertApproxEqAbs(gTranche.trancheBalances(true), delta(seniorTotal, totalWithdrawnSenior), 1E6);
        assertApproxEqAbs(gTranche.trancheBalances(false), delta(juniorTotal,  totalWithdrawnJunior), 1E6);
    }

    function runDeposit(address payable[] memory _users, uint256 deposit, uint256 i, uint256 k) public {
        bool _break;
        for(uint256 j; j < i; j++) {
            ...
            for(uint256 l; l < k; l++) {
                if  (deposit > DAI.balanceOf(user)) {
                    ICurve3Pool(THREE_POOL).add_liquidity([DAI.balanceOf(user), 0, 0], 0);
                } else {
    ...
    function _withdraw(bool tranche, uint256 amount, address user) public returns (uint256 withdrawAmount) {
        (, withdrawAmount) = gTranche.withdraw(amount, 0, tranche, user);
    }

    function userWithdrawCheck(address user, uint256 userSeniorAssets, uint256 userJuniorAssets, uint256 k) public returns (uint256 seniorAmountWithdrawn, uint256 juniorAmountWithdrawn){
        ...
        for(uint256 l; l < k; l++) {
            ...
            vm.startPrank(user);
            if (l + 1 == k) {
                seniorAmountWithdrawn += _withdraw(true, PWRD.balanceOf(user), address(user));
            } else {
                seniorAmountWithdrawn += _withdraw(true, userSeniorAssets / k, address(user));
            }
            assertApproxEqAbs(gTranche.trancheBalances(false), JuniorTrancheAssets, 1E6);


            SeniorTrancheAssets = gTranche.trancheBalances(true);
            if (l + 1 == k) {
                juniorAmountWithdrawn += _withdraw(false, GVT.balanceOf(user), address(user));
            } else {
                juniorAmountWithdrawn += _withdraw(false, userJuniorAssets / k, address(user));
            }
            vm.stopPrank();
            assertApproxEqAbs(gTranche.trancheBalances(true), SeniorTrancheAssets, 1E6);

        }
    }

    function runWithdrawal(address user, uint256 k) public returns (uint256 withdrawnSenior, uint256 withdrawnJunior) {
        ...
        (withdrawnSenior, withdrawnJunior) = userWithdrawCheck(user, userSeniorAssets, userJuniorAssets, k);
