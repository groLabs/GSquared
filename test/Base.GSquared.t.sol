...
contract BaseSetup is Test {
    ...
    function setUp() public virtual {
        ...
        CHAINLINK_AGG_ADDRESSES[0] = address(0xAed0c38402a5d19df6E4c03F4E2DceD6e29c1ee9); 
        CHAINLINK_AGG_ADDRESSES[1] = address(0x8fFfFfd4AfB6115b954Bd326cbe7B4BA576818f6); 
        CHAINLINK_AGG_ADDRESSES[2] = address(0x3E7d1eAB13ad0104d2750B8863b489D65364e32D); 
    ...
    function genThreeCrv(uint256 amount, address _user) public returns (uint256) {
        ...
        setStorage(_user, USDT.balanceOf.selector, address(USDT), type(uint256).max);
    ...
    function genStable(uint256 amount, address token, address _user) public {
        setStorage(_user, IERC20(token).balanceOf.selector, token, type(uint256).max);
    }

    function manipulatePool(bool profit, uint256 change, address pool, address token) public {
    ...
    function prepareRewards(address _convexPool) public {
		uint256 slot = stdstore
			...
			.find();
		vm.store(_convexPool, bytes32(slot), bytes32(50 * IConvexRewards(_convexPool).rewardRate()));
