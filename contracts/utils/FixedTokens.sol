...
contract FixedTokens {
    ...
    address internal immutable SECOND_TOKEN;
    address internal immutable THIRD_TOKEN;

    uint256 internal immutable FIRST_TOKEN_DECIMALS;
    uint256 internal immutable SECOND_TOKEN_DECIMALS;
    uint256 internal immutable THIRD_TOKEN_DECIMALS;

    address internal immutable JUNIOR_TRANCHE;
    address internal immutable SENIOR_TRANCHE;

    uint256 public constant NO_OF_TOKENS = 3;
    ...
    constructor(address[] memory _yieldTokens, address[2] memory _trancheTokens)
    {
        ...
        SECOND_TOKEN = _yieldTokens[1];
        THIRD_TOKEN = _yieldTokens[2];
        FIRST_TOKEN_DECIMALS = 10**ERC4626(_yieldTokens[0]).decimals();
        SECOND_TOKEN_DECIMALS = 10**ERC4626(_yieldTokens[1]).decimals();
        THIRD_TOKEN_DECIMALS = 10**ERC4626(_yieldTokens[2]).decimals();
        ...
        returns (ERC4626 yieldToken)
    {
        ...
        } else if (_index == 1) return ERC4626(SECOND_TOKEN);
        else {
            return ERC4626(THIRD_TOKEN);
        }
    }
        ...
        returns (uint256 decimals)
    {
        ...
        } else if (_index == 1) return SECOND_TOKEN_DECIMALS;
        else {
            return THIRD_TOKEN_DECIMALS;
        }
    }
